...
void liballoc_dump()
{
    ...
    INFO("liballoc", "System memory allocated: %i bytes", l_allocated);
    INFO("liballoc", "Memory in used (malloc'ed): %i bytes", l_inuse);
    INFO("liballoc", "Warning count: %i", l_warningCount);
    INFO("liballoc", "Error count: %i", l_errorCount);
    INFO("liballoc", "Possible overruns: %i", l_possibleOverruns);
...
void *PREFIX(malloc)(size_t req_size)
{
    ...
    if (l_memRoot == NULL)
    {
        ...
        {
            liballoc_unlock();
            ERROR("liballoc", "initial l_memRoot initialization failed", p);
            return NULL;
        }
